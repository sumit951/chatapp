...
const Chatnav = ({sendDataToParent,senderUserData,alluserdata}) => {
    //console.log(senderUserData);
    const chatboardUserId = senderUserData.id
    ...
    const fetchGrouplist = async (chatboardUserId) => {
    try {
            
            //console.log(chatboardUserId);
            const encodeSelectedUserId = btoa(chatboardUserId)
            const response = await axiosConfig.get(`/chat/getgroupselecteduserlist/${encodeSelectedUserId}`)
            if(response.status==200)
            {
                //const token = localStorage.getItem(token)
                if(response.status !== 200)
    ...
    const newUserslisting = alluserdata.filter(item => item.userName !== senderUserData.name);
    const newUsersloggedin = usersloggedin.filter(item => item.userName !== senderUserData.name);
                ...
                <div className="modal-content">
                    <div className="chat-header">
                        ...
                        <ul className="nav nav-tabs" id="myTab" role="tablist">
                            ...
                            <li className="nav-item" role="presentation">
                                <button className="nav-link" id="Closed-tab" data-bs-toggle="tab" data-bs-target="#Closed" type="button" role="tab" aria-controls="Closed" aria-selected="false" onClick={(e)=>fetchGrouplist(chatboardUserId)}><FontAwesomeIcon icon={faUsers} size="1x" />  Group </button>
                            </li>
                        </ul>
                    </div>
